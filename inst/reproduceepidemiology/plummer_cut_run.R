...
# load packages
require(unbiasedmcmc)
require(doParallel)
require(doRNG)
# req_packages <- c("unbiasedmcmc", "doParallel", "doRNG")
...
Npart <- c(
  111, 71, 162, 188, 145, 215, 166, 37, 173,
  143, 229, 696, 93
)
J <- 13
...
Npop <- c(
  26983, 250930, 829348, 157775, 150467, 352445, 553066,
...
get_kernels <- function(theta1, Sigma_proposal, init_mean, init_Sigma) {
  ...
  coupled_kernel <- function(state1, state2) {
    ...
    return(list(
      state1 = list(chain_state = chain_state1, current_pdf = current_pdf1),
